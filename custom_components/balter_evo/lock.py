...
from .const import CONF_DOOR_PIN, DOMAIN, OEM_ID_COMPACT, RELOCK_DELAY
from .p2p import P2PDoorBusyError, async_p2p_open_door
...
class BalterDoorLock(LockEntity):
    ...
    async def async_unlock(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            # laesst die Sitzung dann stumm verfallen -- das saehe wie eine
            # besetzte Station aus und verschleierte den echten Grund.
            if not creds.get("dynamic_password"):
                raise HomeAssistantError(
                    "Die Zugangsdaten der Türstation konnten nicht aus der Balter-Cloud "
                    "geladen werden. Bitte Internetverbindung prüfen; bei anhaltendem "
                    "Fehler die Integration neu anmelden."
                )
            ...
            # Eine falsch eingetragene PIN ist von aussen nicht zu erkennen: das
            # Geraet nimmt den OPENDOOR-Frame entgegen und quittiert ihn einfach
            # nie. Die Cloud kennt den richtigen Hash, also hier vergleichen und
            # den Verdacht ins Log schreiben, statt spaeter zu raten.
            expected_hash = creds.get("out_auth_code") or ""
            if self._pin and expected_hash:
                own_hash = hashlib.sha256(self._pin.encode("utf-8")).hexdigest()
                if own_hash != expected_hash:
                    _LOGGER.warning(
                        "Die in Home Assistant hinterlegte Tür-PIN passt "
                        "nicht zum out-auth-code der Türstation. Das Gerät wird das "
                        "Öffnen vermutlich stillschweigend verweigern. PIN in den "
                        "Integrationsoptionen korrigieren oder leer lassen -- dann "
                        "wird der Hash aus der Cloud verwendet."
                    )
        ...
        except P2PDoorBusyError as err:
            # Der Befehl ging nachweislich nie raus -> gefahrlos erneut versuchbar.
            _LOGGER.warning("P2P door unlock could not start: %s", err)
            raise HomeAssistantError(
                "Die Türstation ist gerade noch mit einer vorherigen Sitzung "
                "beschäftigt und hat die Verbindung nicht freigegeben. Bitte etwa "
                "30 Sekunden warten und dann EINMAL erneut öffnen -- schnelle "
                "Wiederholungen halten die Station nur länger besetzt."
            ) from err
        except HomeAssistantError:
            raise
        except Exception as err:  # noqa: BLE001 -- Netz-/Protokollfehler dem Nutzer zeigen
            _LOGGER.error("P2P door unlock failed: %s", err)
            raise HomeAssistantError(f"Türöffnen fehlgeschlagen: {err}") from err

        # Das Geraet hat das Oeffnen echt quittiert. Fuer Nutzer, die aus der Ferne
        # oeffnen und den Summer nicht hoeren koennen, ist die HA-UI die einzige
        # Rueckmeldung -- darum zusaetzlich eine kurze Bestaetigung einblenden.
        _LOGGER.info("Door unlock confirmed by device for %s (%s)", self.name, duid)
        persistent_notification.async_create(
            self.hass,
            f"{self.name}: Öffnen-Befehl von der Türstation bestätigt.",
            title="Balter EVO",
            notification_id=f"{DOMAIN}_unlock_{self._attr_unique_id}",
        )
    ...
    async def async_will_remove_from_hass(self) -> None:
        """Cancel the pending optimistic re-lock."""
        self._cancel_relock()
