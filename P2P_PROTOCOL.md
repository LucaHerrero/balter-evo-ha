...
## 11. Cred-KDF vollständig geknackt — eigene Client-ID ohne Registrierung (2026-08-23)

Die letzte Lücke ist geschlossen: der `mst/query`-Discovery-Dienst verschlüsselt
die MQTT-`username`/`password` (server-type `p2papp`) mit einem **client-id-
spezifischen** Schlüssel. Dieser wird komplett offline aus der client-id abgeleitet
(`custom_components/balter_evo/qv_kdf.py`), reverse-engineert aus `libqv-p2p-v2.so`
(`tdkcloud::MstEncrypt` + `AESSecret::GenerateKey`).

### Die KDF (byte-genau verifiziert)

```
seed  = client_id, rechts mit '0' auf 32 Zeichen aufgefüllt
box[0]  = B[0xF1];   box[i] = B[(box[i-1] % 16) * box[i-1]]      i = 1..31
box[i] += seed[i]                                                i = 0..31
sbox[0] = A[box[0]]; sbox[j] = A[(sbox[j-1] % 16) * sbox[j-1]]   j = 1..255
# AES-256-Key-Schedule um zwei Wörter erweitern (RotWord/SubWord über die
# dynamische sbox, Rcon(1)); key = box[8:32] ++ w8 ++ w9   (32 Byte)
username = AES-256-CBC-decrypt(base64(feld), key, IV = b"0"*16)  ->  "B_<client_id>"
```

`A` und `B` sind feste, geräteunabhängige 4-KB-Tabellen (aus `GenerateSourceData`);
sie liegen komprimiert in `qv_kdf.py`. Verifiziert: der abgeleitete Schlüssel
reproduziert den per Frida ausgelesenen Original-Schlüssel byte-genau und
entschlüsselt die echten Credentials zu `B_<client_id>` (client-id + Präfix `B_`).

### Konsequenz: keine Registrierung nötig

Der frühere Befund „eine frische ID wird ignoriert" war ein Artefakt des **falschen
festen Schlüssels** — `mst/query` liefert für **jede** client-id gültige, mit der
KDF entschlüsselbare Credentials. Damit funktioniert eine **selbst erzeugte**
16-Hex-ID end to end: Cloud-Login, MQTT `register`+`p2pconnect` und der P2P-Login
am Gerät. Live verifiziert — Standbild und Videostrom mit einer frei gewürfelten ID,
ohne Telefon, ohne App, ohne die Android-SSAID und ohne den `client-login`-Flow.

Die client-id IST die Android-SSAID der App-Installation (siehe §10.5), aber die
Integration braucht sie nicht: sie erzeugt eine eigene und leitet alles davon ab.
